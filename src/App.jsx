...
import { 
  ...
  LayoutDashboard, Dumbbell, Utensils, Trophy, Sparkles, Activity, ArrowRight 
} from 'lucide-react';
  ...
  return (
    <div className="h-screen w-full flex items-center justify-center bg-black p-6 relative overflow-hidden">
      <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} className="w-full max-w-sm bg-[#111113] p-8 rounded-[32px] border border-zinc-800 shadow-2xl relative z-10">
        <h2 className="text-xl font-extrabold text-center tracking-tight mb-8 text-white">{isLogin ? 'Connexion' : 'Rejoindre M CANIK'}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full py-3.5 bg-blue-600 rounded-2xl font-bold text-sm text-white active:scale-95 transition-transform">{isLogin ? 'Entrer' : 'Créer mon compte'}</button>
        </form>
...
function AppRouter() {
  ...
  const [showUpdateNote, setShowUpdateNote] = useState(() => localStorage.getItem('mecanik_update_v4_0') !== 'true');
