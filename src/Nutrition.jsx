...
export default function Nutrition({ onBack, dataContext }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full w-full bg-black text-white relative overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto px-4 pt-6 pb-32 space-y-6">
        <section className="bg-[#151517] rounded-[32px] p-6 border border-[#222225] shadow-2xl">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex flex-col items-center gap-2 cursor-pointer active:scale-90" onClick={syncWorkoutActivity}>
        ...
        <section className="space-y-3">
          {[ { id: 'breakfast', name: 'Petit-déjeuner', icon: Coffee }, { id: 'lunch', name: 'Déjeuner', icon: Utensils }, { id: 'dinner', name: 'Dîner', icon: Moon }, { id: 'snacks', name: 'Snacks', icon: Cookie } ].map(meal => (
            <div key={meal.id} onClick={() => setActiveMealModal(meal.id)} className="bg-[#151517] border border-[#222225] rounded-[24px] p-4 flex items-center justify-between active:scale-95 cursor-pointer">
      ...
      <AnimatePresence>
        {showProfileModal && (
          <ProfileModal 
              profile={profile} setProfile={setProfile} targetGoals={targetGoals} metabolicStats={metabolicStats} 
              onClose={() => setShowProfileModal(false)} onLogout={logout} 
          />
        )}
        {foodToQuantify && (
        ...
        {showContributeModal && (
           <ContributeFoodModal 
              initialBarcode={scannedBarcode} onClose={() => setShowContributeModal(false)} 
              onFoodAdded={(addedFood) => { setGlobalFoodDB(prev => [addedFood, ...prev]); setFoodToQuantify(addedFood); setShowContributeModal(false); }} 
           />
        )}
      </AnimatePresence>
